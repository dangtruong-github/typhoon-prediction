...
from data.pastdomain.merra2_dataset import Merra2_Dataset
...
class Merra2_past_Loader(L.LightningDataModule):
    ...
    def prepare_df(self):
        ...
        df_filtered.loc[df_filtered['Step'].between(0, self.pos_step), 'Label'] = 1
        ...
        train_val_df = self.split_set(df_filtered, "train_val")
        self.df_test = self.split_set(df_filtered, "test")
    ...
    def split_set(self, df_in, type_dataset):
        # Step 3: Train/Val and Test Split by Year
        if type_dataset == "test":
            df_out = df_in[df_in['Year'].between(2017, 2022)]
        elif type_dataset == "train_val":
            df_out = df_in[df_in['Year'].between(1980, 2016)]
            df_out = undersample_data(df_out, ratio=self.rus)
