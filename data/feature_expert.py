...
from data.merra2_base import MerraBasePast, MerraBaseFullMap
...
class MerraFeatureExpertFullMap(MerraBaseFullMap):
    def __init__(self, 
        ...
        type_dataset: str = "train", pos_step: int = 0,
        rus: float = 2
    ):
        # print("HERE")
        # assert 0 == 1
        ...
        pos_steps = [pos_step]
        neg_steps = np.arange(pos_step+1, 41)

        print(pos_steps)

        super().__init__(type_dataset, merra_path, agg_step, agg_alpha, pos_steps, neg_steps, rus)

        if stat_path is None:
            stat_path = '/N/slate/tnn3/TruongChu/merraRun/datasets/csv/data_statistics.xlsx'

        self.T_VAR = ['RH', 'T', 'T', 'H', 'OMEGA', 'U', 'U', 'V', 'V']
        self.T_IDX = [10, 4, 16, 16, 16, 8, 22, 8, 22]
        self.T_CAT = ['RH_750', 'T_900', 'T_500', 'H_500', 'OMEGA_500', 'U_800', 'U_200', 'V_800', 'V_200']
    ...
    def load_data_each(self, nc_path):
        if os.path.isfile(nc_path) is False:
            return np.zeros((9, 61, 81))
        
        save_path = get_save_path(nc_path, type_save="full_map")
        ...
        input_arr = []
        for var, idx in zip(self.T_VAR[:], self.T_IDX[:]):
            arr = ds.variables[var].data[idx]
...
class MerraFeatureExpertPastModule(pl.LightningDataModule):
    def __init__(self,
...
class MerraFeatureExpertFullMapModule(pl.LightningDataModule):
    def __init__(self,
        batch_size=64, rate_under_sampling: float = 1,
        agg_step: int=0, pos_step: int=0
    ):
    ...
    def setup(self, stage=None):
        self.train_set = MerraFeatureExpertFullMap(
            type_dataset="train", agg_step=self.agg_step,
            pos_step=self.pos_step, rus=self.rate_under_sampling
        )
        self.val_set = MerraFeatureExpertFullMap(
            type_dataset="val", agg_step=self.agg_step,
            pos_step=self.pos_step, rus=self.rate_under_sampling
        )
        self.test_set = MerraFeatureExpertFullMap(
            type_dataset="test", agg_step=self.agg_step,
            pos_step=self.pos_step, rus=self.rate_under_sampling
        )
