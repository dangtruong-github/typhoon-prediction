import sys
sys.path.insert(0, '/N/slate/tnn3/DucHGA/Lib')
...
class Merra2_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.pre_load:
            ...
            input = self.get_data(self.data_path.iloc[idx])
...
class Merra2_time_dataset(Merra2_dataset):
    ...
    def __getitem__(self, idx):
        ...
        _time = self.data_path.iloc[idx]['Hour'] / 24
        _time = torch.tensor(_time, dtype=torch.float)
        
        label = self.data_path.iloc[idx]['Label']
        label = torch.tensor(label, dtype=torch.float).type(torch.LongTensor)
        
        return _time, input, label

class Merra2_full_dataset(Merra2_dataset):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def get_data(self, sample):
        ...
        def read_data(row):
            ...
            # res[np.isnan(res)] = np.take(self.max * 2, np.isnan(res).nonzero()[0])
            input = np.array(input)[:, 100: 161, 64: 145]
            res = (np.array(input) - self.mean) / self.std * weight # norm
            res[np.isnan(res)] = 0
...
class Merra2_ck_dataset(Merra2_dataset):
    def __init__(self, 
                 merra_path: Path,
                 stat_path: Path = '/N/slate/tnn3/DucHGA/TC2/DataMerra/Data/data_train_statistics.xlsx',
                 dtype: str = 'clf',
...
class Merra2_filter_dataset(Merra2_dataset):
    def __init__(self, 
        ...
        # self.T_VAR = ['OMEGA', 'QL', 'QL', 'QL', 'QL', 'OMEGA', 'QL', 'QL', 'QL', 'QL', 'QL', 'QL', 'QL', 'QL', 'QL', 'RH', 'OMEGA', 'OMEGA']
        # self.T_PRS = [1000, 950, 925, 975, 900, 100, 700, 800, 875, 775, 750, 850, 825, 725, 650, 925, 975, 150]
        
        self.T_VAR = ['OMEGA', 'QL', 'OMEGA', 'QL', 'QL', 'RH', 'OMEGA', 'RH', 'QL', 'V', 'RH', 'QL', 'QL', 'V', 'U', 'QV', 'OMEGA', 'QI', 'QL', 'T', 'QI', 'QI']
        self.T_PRS = [1000, 925, 100, 700, 800, 925, 150, 1000, 600, 400, 150, 1000, 400, 1000, 1000, 100, 550, 500, 100, 200, 650, 1000]        
        # self.T_LIST = pd.read_excel()
        # self.T_CAT = ['RH750', 'T900', 'T500', 'H500', 'OMEGA500', 'U800', 'U200', 'V800', 'V200', 'VOR900', 'VOR700', 'VOR200', 'DIV200']
        # self.T_IDX = [10, 4, 16, 16, 16, 8, 22, 8, 22, 4, 12, 22, 22]
        
        
        self.features = pd.read_excel("/N/slate/tnn3/DucHGA/ranking_score_filter.xlsx")[["var",f"corr_{corr}"]]
...
class Merra2_mae_dataset(Merra2_dataset):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
...
#===================    
def prepare_Dataset(DatasetClass = Merra2_time_dataset,
    ...
    #     save_kwargs['DatasetClass'] = DatasetClass.__name__
    #     json.dump(save_kwargs, open(os.path.join(out_dir, 'config', 'Dataset.json'), 'w'))
    #     print('[INFO]: Dataset config saved!')
...
if __name__ == '__main__':
    trainLoader, valLoader, testLoader = prepare_Dataset(DatasetClass = Merra2_filter_dataset,
                    train_path = '/N/slate/tnn3/DucHGA/TC2/DataMerra/Data/sample_path.csv',
                    val_path = '/N/slate/tnn3/DucHGA/TC2/DataMerra/Data/sample_path.csv',
                    test_path = '/N/slate/tnn3/DucHGA/TC2/DataMerra/Data/sample_path.csv',
                    num_workers = 64,
                    corr = 0.3,
                    pre_load = True,)
    
    print(getsizeof(trainLoader))
    print(getsizeof(valLoader))
    print(getsizeof(testLoader))
    
    for yahoo in trainLoader:
        pass